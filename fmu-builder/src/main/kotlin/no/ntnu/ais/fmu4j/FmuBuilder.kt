...
class FmuBuilder(
    ...
    fun build(dest: File? = null): File {
        ...
        val superClass = classLoader.loadClass("no.ntnu.ais.fmu4j.export.fmi2.Fmi2Slave")
        val subClass = classLoader.loadClass(mainClass)
        val instance = subClass.getConstructor(Map::class.java).newInstance(fmuArgs)

        val mdClass = classLoader.loadClass("no.ntnu.ais.fmu4j.modeldescription.fmi2.Fmi2ModelDescription")
        val mdCsClass = classLoader.loadClass("no.ntnu.ais.fmu4j.modeldescription.fmi2.Fmi2ModelDescription\$CoSimulation")
